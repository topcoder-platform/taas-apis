...
const updateRolesWithTcUserIds = async () => {
    ...
    const recordsToProcess = await models.WorkPeriod.findAll({
    ...
    try {
        // update work_periods with tc user ids
        ...
        console.log('Successfully updated work_periods with tcUserIds')
        process.exit(0)
    } catch (err) {
        // rollback the transaction if an error occurs
        await transaction.rollback()

        console.log('An error happened when updating work_periods with tcUserIds')
        console.log(err)
        process.exit(1)
    }
}
