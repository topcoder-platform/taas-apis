...
const updateRolesWithTcUserIds = async () => {
  ...
  const recordsToProcess = await models.WorkPeriodPayment.findAll({
  ...
  try {
    // update work_period_payment with tc user ids
    for (const record of recordsToProcess) {
      ...
      if (MODE === 'test') {
        ...
        console.log(`Updating work_period_payment with id ${record.id} with createdBy ${createdByTcUserId} and updatedBy ${updatedByTcUserId}`)
      } else {
        await record.update({
    ...
    console.log('Successfully updated work_period_payment with tcUserIds')
    process.exit(0)
  } catch (err) {
    // rollback the transaction if an error occurs
    await transaction.rollback()

    console.log('An error happened when updating work_period_payment with tcUserIds')
    console.log(err)
    process.exit(1)
  }
}
